...
/**
 * The identity behind one authenticated `/mcp` request. `id` is the isolation key (stable per
 * user); `email` is what gets written as a draft's `createdBy`.
 *
 * There is no `kind`. `/mcp` authenticates exactly one sort of caller — a signed-in human over
 * OAuth (ADR-0002, ADR-0008) — so there is no second issuer to tell apart, nothing to filter the
 * tool list by, and no principal whose reach a claim or a capability has to scope.
 */
 ...
 *
 * ADR-0005 once added a SECOND ISSUER here — a provisioned credential for an unattended drainer.
 * ADR-0008 removed it along with the queue that needed it: repair is attended, so the only caller
 * is a person. A bearer token that is not a user's OAuth token is refused with 401, with no second
 * issuer to fall through to.
 *
 * The Bridge Helper is unaffected, and always was: its spawned Claude reaches Varys as the USER
 * over OAuth, which is what makes a Run started by pressing Run attributable to the person who
 * pressed it.
 */
@Injectable()
