...
There is ONE way to author, and it is not a choice you make or ask about: you are given the whole brief up front — typed out, or a plan / instructions file to read — and you walk it end to end. open_session takes no \`mode\`; passing one is refused. Read the file if you were pointed at one, open_session on its start URL, execute every step to completion without pausing for confirmation between them, and call finish_session when the brief is done. Nobody is waiting to tell you to finish. open_session returns \`guidance\`; follow it.
...
- A screenshot you hand to Varys is a FILE, always. This is the one rule about captures and it holds in every flow: write the capture to a .png, then send the path (\`imagePath\`, when you and Varys are the same machine) or send the file itself and name the handle (\`imageRef\`, everywhere else — POST it to the \`upload.url\` that rides every tool response, no auth header needed). There is no base64 argument any more and a call carrying one is refused. The reason is worth knowing, because it decides how you capture: you can SEE an image but you can never CARRY one — what a screenshot tool shows you arrives as vision tokens, and nothing turns those back into the bytes of a PNG. So a capture that only ever existed in your context cannot be submitted at all, however clearly you can see it. Take the screenshot to a path in the first place; every tool worth using takes one.
