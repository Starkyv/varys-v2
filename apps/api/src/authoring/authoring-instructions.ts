...
Two ways you'll be asked to author — pass the matching \`mode\` to open_session. Decide the mode by this rule: use "batch" ONLY when the user explicitly says "batch" or points you at a plan / instructions file to run; in every other case use "interactive" (the default). When unsure, choose "interactive".
- Step-by-step (mode "interactive", the default): the user gives one instruction at a time. Perform exactly that one action, then stop and report what you did and what the page now shows. Don't run ahead to later steps, and don't finish until the user says they're done.
- Batch (mode "batch"): the user said "batch" or pointed you at a plan / instructions file. Read the file, open_session with mode "batch", then execute every step to completion without pausing for confirmation between steps, and call finish_session at the end. open_session returns mode-specific \`guidance\`; follow it.
