...
describe("Suites API", () => {
  ...
  it("replaces a suite's member list wholesale on update", async () => {
    ...
    const before = await authed(app).get(`/suites/${suiteId}`).expect(200);
    ...
    const after = await authed(app).get(`/suites/${suiteId}`).expect(200);
    ...
    const listed = await authed(app).get("/suites").expect(200);
    expect(
  ...
  it("deleting a suite leaves its member tests intact (a test can be in many suites)", async () => {
    ...
    const s1 = await authed(app)
      .post("/suites")
      .send({ name: "release", testIds: [shared] })
      .expect(201);
    const s2 = await authed(app)
      .post("/suites")
      .send({ name: "nightly", testIds: [shared] })
      .expect(201);

    await authed(app).delete(`/suites/${s1.body.id}`).expect(200);
    await authed(app).get(`/suites/${s1.body.id}`).expect(404);
